            ...
            <div className="container flex h-16 items-center justify-evenly mx-auto">
                <div className="flex items-center gap-6">
                    ...
                    <div className="hidden md:flex gap-1">
                        {navItems.map((item) => (
                            <Button key={item.href} variant={item.active ? "secondary" : "ghost"} size="sm" className={cn("gap-2", item.active && "bg-muted")} asChild>
                                <Link
                                    href={item.href}
                                    onClick={() => {
                                ...
                                >
                                    {isPending && item.href === clickedHref ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-background border-t-foreground" /> : <item.icon className="h-4 w-4" />}
                                    {item.label}
                                </Link>
                            </Button>
                        ))}
                            ...
                            <div className="flex flex-col gap-2">
                                {navItems.map((item) => (
                                    <Button
                                        key={item.href}
                                        variant="ghost"
                                        className={cn("justify-start gap-2", item.active && "bg-muted")}
                                        onClick={() => {
                                            setClickedHref(item.href);
                                            startTransition(() => {
                                                setIsOpen(false);
