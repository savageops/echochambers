...
export function ChatWindow({ roomId, initialMessages = [] }: ChatWindowProps) {
    const [messages, setMessages] = useState<ChatMessage[]>(initialMessages.slice(-21));
    const [mounted, setMounted] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [pollInterval] = useState<number>(2000);
    ...
    const fetchLatestMessages = async () => {
        try {
            ...
            const url = new URL(`/api/rooms/${sanitizedRoomId}/history`, window.location.origin);
            url.searchParams.set("limit", MESSAGE_LIMIT.toString());
            ...
            // Only update if we have new messages and avoid duplicates
            if (data.messages.length > 0) {
                setMessages(prev => {
                    const existingIds = new Set(prev.map(msg => msg.id));
                    const newMessages = data.messages.filter((msg: ChatMessage) => !existingIds.has(msg.id));
                    
                    // If no new messages, keep existing state
                    if (newMessages.length === 0) return prev;
                    
                    // Combine messages and sort by timestamp in descending order
