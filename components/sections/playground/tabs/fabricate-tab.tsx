...
export function FabricateTab({ onConversationChange }: FabricateTabProps) {
    const [messages, setMessages] = useState<Message[]>(() => {
        ...
        if (typeof window !== "undefined") {
            const saved = localStorage.getItem(STORAGE_KEYS.FABRICATIONS);
            if (saved) {
                try {
    ...
    useEffect(() => {
        localStorage.setItem(STORAGE_KEYS.FABRICATIONS, JSON.stringify(messages));
    }, [messages]);
        ...
        <div className="h-[400px] space-y-4">
            <div className="flex flex-col gap-4 h-full">
                        ...
                        <Card key={index} className="mb-4 p-3 bg-muted/50">
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <Label className="text-sm font-medium">{message.role === "user" ? "User" : "Assistant"}</Label>
                                </div>
                                <div className="flex items-center gap-1">
                                    <Button variant="ghost" size="icon" onClick={() => moveMessage(index, "up")} disabled={index === 0}>
                                        <MoveUp className="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => moveMessage(index, "down")} disabled={index === messages.length - 1}>
                                        <MoveDown className="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => duplicateMessage(index)}>
                ...
                <div className="space-y-4">
                    <Textarea value={currentContent} onChange={(e) => setCurrentContent(e.target.value)} placeholder="Type your message here..." className="h-[100px] min-h-[100px] max-h-[200px] bg-background/50" />
                    <div className="flex flex-col md:flex-row gap-2">
                        <Button onClick={() => addMessage("user")} className="flex-1" disabled={!currentContent.trim()}>
                            <User2 className="h-4 w-4 mr-2" />
                            Add User Message
                        </Button>
                        <Button onClick={() => addMessage("assistant")} className="flex-1" disabled={!currentContent.trim()}>
