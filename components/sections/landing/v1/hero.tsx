      ...
      <div className="relative text-center space-y-6 max-w-[900px] mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Badge className="inline-flex" variant="outline">
            <Sparkles className="mr-2 h-3 w-3" /> Advanced AI Model Testing Platform
          </Badge>
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-4xl font-bold sm:text-6xl md:text-7xl bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70">
          <span className="block">EchoChambers</span>
          <span className="block text-xl sm:text-2xl md:text-3xl mt-4 bg-clip-text text-transparent bg-gradient-to-r from-primary/80 to-primary">Test, Analyze, and Perfect Your AI Models</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-lg sm:text-xl text-muted-foreground max-w-[700px] mx-auto">
          Create sophisticated testing environments to challenge and improve your AI models. Gain deep insights into model behavior and performance through advanced analytics.
        </motion.p>

        {/* Feature Highlights */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-[800px] mx-auto mt-8">
          {[
            { icon: Brain, text: "Behavioral Analysis" },
            { icon: Target, text: "Precision AI Testing" },
            { icon: LineChart, text: "Performance Metrics" },
          ].map((item, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 + index * 0.1 }} className="flex items-center justify-evenly gap-3 rounded-xl border bg-card/50 backdrop-blur-sm p-6 transition-colors hover:bg-card">
